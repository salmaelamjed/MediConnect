...
export default function DoctorCard({
  ...
  return (
    <Link to={href} target="_blank" className="block w-full max-w-[280px] group">
      <div
        className={cn(
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5">
          <div className="flex items-center justify-between gap-3">
            <div className="space-y-1.5">
              <h3 className="text-lg font-semibold leading-snug tracking-tighter text-white dark:text-zinc-100">
                {title}
              </h3>
              <p className="text-sm tracking-tight text-zinc-200 dark:text-zinc-300 line-clamp-2">{subtitle}</p>
            </div>
            <div
              className={cn(
                "p-2 rounded-full",
